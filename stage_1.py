...
def encrypt(plaintext, key):
    ...
    # 初始置换
    temp_text = permute(ip, plaintext)
    # 第一轮变换
    temp_text = switch(f_k(temp_text, k1))
    # 第二轮变换
    temp_text = f_k((temp_text), k2)
    return permute(ip_inv, temp_text)
...
def decrypt(ciphertext, key):
    ...
    k1, k2 = get_sub_key(key)
    # 初始置换
    temp_text = permute(ip, ciphertext)
    # 第一轮变换
...
# GUI布局和组件初始化
ttk.Label(root, text="明文 (8-bit):").grid(row=0, column=0, sticky="w", padx=10, pady=10)
...
ttk.Label(root, text="密文 (8-bit):").grid(row=3, column=0, sticky="w", padx=10, pady=10)
ttk.Entry(root, textvariable=ciphertext_var).grid(row=3, column=1, padx=10, pady=10)
ttk.Label(root, text="解密文本 (8-bit):").grid(row=4, column=0, sticky="w", padx=10, pady=10)
